...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        sqrt_abar = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrt_1m_abar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t - sqrt_1m_abar * noise) / sqrt_abar
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        sqrt_abar = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrt_1m_abar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - sqrt_abar * x_start) / sqrt_1m_abar
        ####################################################################
        return pred_noise
    ...
    def q_sample(self, x_start, t, noise):
        ...
        # 大约3行代码。
        ####################################################################
        sqrt_abar = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        sqrt_1m_abar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t = sqrt_abar * x_start + sqrt_1m_abar * noise
        ####################################################################
        return x_t
