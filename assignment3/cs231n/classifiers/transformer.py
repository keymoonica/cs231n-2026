...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        cap_emb = self.positional_encoding(cap_emb)

        # 2. 图像特征投影并扩展维度作为 memory (N, 1, W)
        img_feat = self.visual_projection(features).unsqueeze(1)

        # 3. 构造未来掩码: 下三角矩阵, mask=1可见, mask=0屏蔽
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device))

        # 4. Transformer 解码器前向
        dec_out = self.transformer(tgt=cap_emb, memory=img_feat, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 5. 将其通过线性层生成类别logits。                                         #
        ############################################################################
        # 1. 图像分块 + 嵌入 (N, num_patches, embed_dim)
        patch_emb = self.patch_embed(x)
        # 2. 添加位置编码
        patch_emb = self.positional_encoding(patch_emb)
        # 3. Transformer 编码器
        enc_out = self.transformer(src=patch_emb)
        # 4. 全局平均池化
        global_feat = torch.mean(enc_out, dim=1)
        # 5. 分类输出
