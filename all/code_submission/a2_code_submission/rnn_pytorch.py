...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2) 词嵌入：将captions_in从索引转换为向量
        embedded = word_embedding_forward(captions_in, W_embed)
        
        # (3) 根据cell_type选择RNN类型进行前向传播
        if self.cell_type == 'rnn':
            h = rnn_forward(embedded, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            h = lstm_forward(embedded, h0, Wx, Wh, b)
        
        # (4) 使用时序仿射层计算每个时间步的词汇表得分
    ...
    def sample(self, features, max_length=30):
        ...
        # 对于LSTM，还需要初始化细胞状态为0
        H = Wx.shape[1] // {"lstm": 4, "rnn": 1}[self.cell_type]
        if self.cell_type == 'lstm':
            c0 = torch.zeros(N, H, device=features.device, dtype=features.dtype)
        
        # 第一个输入词是<START>标记
        if self._start is None:
            current_input = torch.full((N, 1), self._null, dtype=torch.long, device=features.device)
        else:
            current_input = torch.full((N, 1), self._start, dtype=torch.long, device=features.device)
        
        # 逐步生成每个时间步的词
        for t in range(max_length):
            # (1) 词嵌入
            embedded = word_embedding_forward(current_input, W_embed)
            embedded = embedded.squeeze(1)
            
            # (2) RNN步骤
            if self.cell_type == 'rnn':
                h = rnn_step_forward(embedded, h0, Wx, Wh, b)
                h0 = h
            elif self.cell_type == 'lstm':
                h0, c0 = lstm_step_forward(embedded, h0, c0, Wx, Wh, b)
            
            # (3) 仿射变换得到得分
            scores = temporal_affine_forward(h0.unsqueeze(1), W_vocab, b_vocab)
            scores = scores.squeeze(1)
            
            # (4) 选择得分最高的词
            next_input = torch.argmax(scores, dim=1).unsqueeze(1)
            captions[:, t] = next_input.squeeze(1)
