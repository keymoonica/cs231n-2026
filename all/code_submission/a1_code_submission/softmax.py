...
def softmax_loss_naive(W, X, y, reg):
    ...
    #############################################################################
        ...
        loss -= np.log(p[y[i]])  # 负对数概率即为损失

        for j in range(num_classes):
            indicator = 1 if j == y[i] else 0  # 指示函数，正确类别为1，其他为0
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    # 计算梯度
    # probs 形状 (N, C)，y 形状 (N,)
    # 创建指示矩阵：正确类别位置为 1，其他为 0
    data_prob = probs.copy()
    data_prob[range(num_train), y] -= 1
    dW = X.T.dot(data_prob) / num_train + 2 * reg * W
